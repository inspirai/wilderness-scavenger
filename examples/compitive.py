...
# game setup
parser.add_argument("--timeout", type=int, default=60 * 3)  # The time length of one game (sec)
...
class SupplyBattleMultiAgentEnv(MultiAgentEnv):
    def __init__(self, config: EnvContext):
        ...
        time_stamp = config.get("run","ppo")+"competitive"+f"{self.server_port}"
        ...
        self.episode_num = 0
        self.time_steps = 0
        self.group = {"group1":[0,1],"group2":[2,3]}
    ...
    def reset(self, record=False, replay_suffix=""):
        ...
        self.episode_num+=1
        self.episode_supply_num = {"group1":0,"group2":0}
    ...
    def step(self, action_dict):
        ...
        if self.game.is_episode_finished():
            for id ,value in self.collected_supplys.items():
                if id in self.group["group1"]:
                    self.episode_supply_num["group1"]+=value
                else:
                    self.episode_supply_num["group2"]+=value

            self.writer.add_scalar('group1_episode_supply_num',self.episode_supply_num["group1"],global_step=self.time_steps)
            self.writer.add_scalar('group2_episode_supply_num',self.episode_supply_num["group2"],global_step=self.time_steps)
...
if __name__ == "__main__":
    ...
    def env_creator(env_config):
        env = SupplyBattleMultiAgentEnv(env_config)
        return env.with_agent_groups(groups={"group1":[0,1],"group2":[2,3]},
            obs_space=Tuple([env.observation_space,env.observation_space]),
            act_space=Tuple([env.action_space,env.action_space]))
    register_env("lll",env_creator)
    # state = env.reset()
    # print(state)
    # env.close()
